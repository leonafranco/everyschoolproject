...
public class GereUtilizador {
    ...
    public String procuraEstafeta (String Procurar) {
        String lista ="";
            try {
                ps = conn.prepareStatement("Select * from funcionario, utilizador where u_id = utilizador_u_id AND f_especializacao = ? AND u_estado = 'ativo'");
    ...
    public Utilizador alterarUtilizadorGestor(String login) {
        try {
            ps = conn.prepareStatement("Select * from utilizador where u_login = ?");
    ...
    public Funcionario alterarFuncGestor (String login) {
        long id = u_id(login);
        Utilizador utilizador = alterarUtilizadorGestor(login);
    ...
    public Cliente alterarClienteGestor(String login){
        long id = u_id(login);
        Utilizador utilizador = alterarUtilizadorGestor(login);

        try {
            ps = conn.prepareStatement("Select * from cliente where utilizador_u_id = ?");
            ...
            rs = ps.executeQuery();
            if (rs.next()) {
                long contribuinte = rs.getLong("c_contribuinte");
                long contacto = rs.getLong("c_contacto");
                String morada = rs.getString("c_morada");


                Cliente cliente = new Cliente(utilizador.getNome(), utilizador.getLogin(), utilizador.getPassword(), utilizador.getEstado(), utilizador.getEmail(), utilizador.getTipo(), contribuinte, contacto, morada);
