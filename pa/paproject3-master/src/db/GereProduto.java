...
public class GereProduto {
        ...
        String lista ="";
            try {
                ...
                while (rs.next()) {
                    ...
                    lista += SKU + " | " + designacao + " | " + preco + " | " + stock ;
                }
    ...
    public boolean pesquisarSeExisteStock(long valor, long SKU) {
        try {
            ps = conn.prepareStatement("Select * from produto where p_stock > ? AND p_SKU = ?");
            ps.setLong(1, valor);
            ps.setLong(2, SKU);
            rs = ps.executeQuery();
    ...
    public boolean retirarStock(long valor, long SKU) {

        valor = getStock(SKU) - valor;
        try {
            ps = conn.prepareStatement("UPDATE produto SET p_stock = ? where p_SKU = ?");
            ps.setLong(1, valor);
            ps.setLong(2, SKU);
            ps.executeUpdate();
